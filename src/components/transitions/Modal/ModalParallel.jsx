...
const ModalParallel = React.memo(({ className, children, path, small, lang }) => {
  ...
  const classesBg = isVisible ? `${styles.bg} ${styles["bg__visibility"]}` : `${styles.bg}`;
  const classes = isVisible
    ? `${styles.modal} ${styles.modal__visibility} ${small && styles.modal__small} ${
        className || ""
      }`
  ...
  useEffect(() => {
    if (isVisible === "close" || isVisible === "close-no-refresh" || isVisible === "close-hard") {
      closeModalHandler();
    }
  }, [isVisible]);
  ...
  return (
    <div className={classesBg} onClick={closeModalHandler}>
      <div className={classes} onClick={modalClickHandler} id="modal">
        <IconRender variant="cross" className={styles["modal__quit"]} onClick={closeModalHandler} />
        <ImgBgBlur
