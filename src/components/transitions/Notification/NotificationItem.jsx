...
const NotificationItem = ({
  ...
  const classesHeading = `${styles["notice__heading"]} ${styles[`notice__heading--${action}`]}`;
  const classesIcon = `${styles["notice__icon"]} ${styles[`notice__icon--${action}`]} ${
  ...
  return (
    <li className={styles.notice}>
      <Link href={href ? href : "#"} className={classesLink} onClick={href && closeModalHandler}>
        <div className={styles["notice__content"]}>
          <h3 className={classesHeading}>{lang === "en" ? actionTextEN : actionTextPL}</h3>
